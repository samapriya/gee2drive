...
def imgexp(collection):
    ...
    if typ=="Image":
        ...
        b8uint=[]
        fl=[]
        for key,value in ee.Image(collection).bandTypes().getInfo().items():
        ...
        if len(b16uint)>0:
            print("16 bit unsigned integer bands "+str(sorted_nicely(b16uint)))
        if len(b16sint)>0:
            print("16 bit signed integer bands "+str(sorted_nicely(b16sint)))
        if len(b32uint)>0:
            print("32 bit unsigned integer bands "+str(sorted_nicely(b32uint)))
        if len(fl)>0:
            print("Bands of type Float "+str(fl))
    elif typ=="ImageCollection":
        info=ee.Image(ee.ImageCollection(collection).first()).getInfo()
        b16uint=[]
        b32uint=[]
        b16sint=[]
        b8uint=[]
        fl=[]
        for key,value in ee.Image(ee.ImageCollection(collection).first()).bandTypes().getInfo().items():
            if value['precision']=="float":
                fl.append(str(key))
