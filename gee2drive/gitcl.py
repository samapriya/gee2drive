...
if os.path.exists(pth):
    if name=='nt':
        ...
        try:
            ...
            #os.system('sudo rm -r '+'"'+pth+'"')
    
Repo.clone_from('https://github.com/samapriya/Earth-Engine-Datasets-List.git', os.path.join(src,folder_name))
...
for items in os.listdir(os.path.join(src, folder_name)):
    if items.endswith('.csv'):
        ...
        with open(os.path.join(src,items)) as fp:
            ...
            data = list(reader)
            row_count = len(data)
            print('EE dataset updated and has '+str(row_count)+' assets')
